...
--bigCategry
drop table if exists big_category cascade;
create table big_category(
 id serial not null 
,name character(255)
,constraint big_categry_PKC primary key (id)
);

--mediamCategory
drop table if exists mediam_category cascade;

create table mediam_category(
 id serial not null 
,name character(255)
, big_category_id integer
,FOREIGN KEY(big_category_id) REFERENCES big_category(id)
,constraint mediam_category_PKC primary key (id)) ;

--smalCategory
drop table if exists smal_category cascade;

create table smal_category(
 id  serial not null 
,name character(255)
,mediam_category_id integer 
,name_all character varying(255)
,FOREIGN KEY(mediam_category_id) REFERENCES mediam_category(id)
,constraint smal_category_PKC primary key (id)) ;
...
create table items (
  ...
  , name character varying(255)
  , condition integer
  , smal_category_id integer
  , brand character varying(255)
  , price double precision
  , shipping integer
  , description text
  , constraint items_PKC primary key (id)
  ,FOREIGN KEY(smal_category_id) REFERENCES smal_category(id)) ;
...
FOREIGN KEY(ancestor) REFERENCES big_category(id)
);
